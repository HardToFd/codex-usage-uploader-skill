---
name: codex-usage-uploader
description: Upload sanitized Codex Desktop or CLI usage metadata to a configured ingest API for dashboards, quota tracking, and multi-source usage reporting. Use when Codex needs to configure, run, backfill, troubleshoot, or automate local Codex session JSONL collection with only an endpoint, source name, and Bearer token.
---
...
Use this skill to collect local Codex session JSONL usage metadata and upload it to a dashboard ingest API. The bundled script performs incremental scanning, privacy filtering, deterministic event IDs, Bearer-token upload, and local state tracking.
...
```bash
set CODEX_USAGE_INGEST_URL=https://collector.example.com/api/codex/usage
...
```bash
python scripts/codex_usage_uploader.py --dry-run --endpoint https://collector.example.com/api/codex/usage --source-name alice
python scripts/codex_usage_uploader.py --since 2026-05-01 --endpoint https://collector.example.com/api/codex/usage --source-name alice --token <bearer-token>
python scripts/codex_usage_uploader.py --codex-home C:\Users\admin\.codex --timezone Asia/Shanghai --batch-size 500 --endpoint https://collector.example.com/api/codex/usage --source-name alice --token <bearer-token>
```
...
```text
Use $codex-usage-uploader to run the Codex usage uploader with the configured CODEX_USAGE_INGEST_URL, CODEX_USAGE_SOURCE_NAME, and CODEX_USAGE_BEARER_TOKEN environment variables. Upload new local Codex usage metadata only.
```
...
- Duplicate data: expected during file moves or re-scans; de-duplicate by `event_id`.
